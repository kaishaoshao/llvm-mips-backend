...
```
The `dag patternToMatch` is the SDNode that we want to match. This is also called as an
*input pattern*.
The `list<dag> resultInstrs` is the list of result SDAGs that we want to generate
on matching the previous pattern. This is called as the *output pattern*.
...
#### Pattern syntax
I have attempted to condense the allowed syntax of patterns in this 
BNF-like grammar. This is not exhaustive, but should be enough to serve as a
quick reference.

> On your first read, you can skip this block below and jump to the explanations ahead.
...
PatternOperator ::= 'null_frag'
                    | TypeCastOperator
                    | SDNodeOperator
                    | recordOfType( PatFrags
                                    | Instruction | SDNodeXForm // both can be in output patterns only
                                    | Intrinsic // in input patterns only
                                    | ComplexPattern
                                  )
                    | 'set' // can only be used in instruction inline patterns
                    | 'ops' | 'ins' | 'outs' // only for pattern fragment operators 

SDNodeOperator ::= OutputSDNodeOperator // for output patterns only
                   | InputSDNodeOperator // for input patterns only
...
OutputSDNodeOperator ::= 'imm' | 'timm' | 'fpimm' | 'tglobaltlsaddr'
                    ...
                    | 'bb' | 'vt' | 'mcsym' // no other nodes cannot be emitted in the DAG
...
PatternArg ::= PatternArgVal ArgName? // argument can have an optional name
            ...
            | instanceOfType(TGTypes::Bits) ArgName?
                                           ...
                                           // all are emitted as TargetConstant nodes.
...
PatternArgVal ::= pattern // note: (add imm:$imm) is the same as (add (imm):$imm)
                ...
                | 'node' // special record to indicate an input argument
                         // for a PatFrag
```
...
patterns or other pattern fragments. These are to de-duplicate common patterns.
...
`node` is a special record that can only be used in pattern fragments.

```cpp title="Using the patfrag"
def : Pat<(not i16:$in), (NotInstr i16:$in)>;
// This will be inlined to:
