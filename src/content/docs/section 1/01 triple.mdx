...
But read on, because we will add our target in this chapter.
</Aside>
...
Now we can run `llc` with the `mipsnova` target.
...
This won't work yet because we haven't implemented the target machine for `mipsnova`,
